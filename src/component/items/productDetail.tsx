...
function ProductDetail() {
  ...
  const getRateStar = (rateNumber: number) => {
    ...
    for (let i = 0; i < starCount; i++) {
      if (i % 2 === 0) {
        stars.push(<input key={i} type="radio" name="rating-10" className="star" disabled checked />);
      } else {
        stars.push(<input key={i} type="radio" name="rating-10" className="star2" disabled checked />);
      }
    }
    ...
    if (rateNumber < 5) {
      for (let i = starCount; i < 10; i++) {
        if (i % 2 === 0) {
          stars.push(<input key={i} type="radio" name="rating-10" className="emstar" disabled />);
        } else {
          stars.push(<input key={i} type="radio" name="rating-10" className="emstar2" disabled />);
        }
      }
    }
          ...
          <div className="detailConatiner">
            ...
            <div className="productDetail">
              <h2 className="productDetailTitle">
                {product.title} <span className="New">NEW</span>{" "}
              </h2>
